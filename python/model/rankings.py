...
def make_rankings(games, oos_games, ranker_start = 30):
    ...
    elos, current_ratings = elo.compute(df)
    df = df.drop(columns=['result'])
    df['home_team_rating'] = elos[::,0]
    df['away_team_rating'] = elos[::,1]
    oos_df['home_team_rating'] = oos_df['home_team'].apply(lambda a: current_ratings[a])
    oos_df['away_team_rating'] = oos_df['away_team'].apply(lambda a: current_ratings[a])
